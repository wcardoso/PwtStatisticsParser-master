...
function includeExample(value, lang) {
    var txt = document.getElementById("statiotext_before");
    if (value == true) {
...
function parseOutput(txt, lang, idPrefix) {
    ...
    var isError = false;
    var formattedOutput = '';
    var tablesList = []; // collected structured table data for external use
        ...
        switch (rowType) {
            ...
            case rowEnum.ExectuionTime:
                if (isExecution === true) {
                    ...
                    // Do not render individual execution time blocks here to avoid repetition
                    // Individual times are accumulated in executionTotal and shown in Totals
            ...
            case rowEnum.CompileTime:
                if (isCompile === true) {
                    ...
                    // Do not render individual compile time blocks here to avoid repetition
                    // Individual times are accumulated in compileTotal and shown in Totals
            ...
            default:
                if (inTable === true) {
                    inTable = false;
                    var totals = statsIOCalcTotals(tableIOResult);
                    formattedOutput += outputIOTable(tableIOResult, totals, tableCount, lang, idPrefix);
                    // push structured data copy
                    tablesList.push({ statInfo: tableIOResult.slice(), statTotal: totals, tableNumber: tableCount });
    ...
    return { output: formattedOutput, tableCount: tableCount, tables: tablesList, executionTotal: executionTotal, compileTotal: compileTotal };
}

// Load language JSON synchronously (used for parsing language-specific labels)
...
function getLangFromText(txt) {
    var lines = txt.split('\n');
    for (var i = 0; i < lines.length; i++) {
        var l = lines[i].trim();
        if (l.length === 0) continue;
        var langIdx = determineLang(l);
        switch (langIdx) {
            case 2: return loadLangFile(determineLangFilename('es'));
            case 3: return loadLangFile(determineLangFilename('it'));
            default: return loadLangFile(determineLangFilename('en'));
        }
    }
    return loadLangFile(determineLangFilename('en'));
}

// Public API: parseStatistics(text) returns structured data: {tables: [...], executionTotal, compileTotal, tableCount, output}
function parseStatistics(text) {
    var lang = getLangFromText(text) || loadLangFile(determineLangFilename('en'));
    return parseOutput(text, lang);
}

function parseText(lang) {
    var before = parseOutput(document.getElementById("statiotext_before").value, lang, 'before_');
    var after = parseOutput(document.getElementById("statiotext_after").value, lang, 'after_');

    var combined = '<div class="row comparison-row">';
    combined += '<div class="col-md-6"><div class="comparison-panel compact"><h3>Antes</h3><div class="comparison-scroll">' + before.output + '</div></div></div>';
    combined += '<div class="col-md-6"><div class="comparison-panel compact"><h3>Depois</h3><div class="comparison-scroll">' + after.output + '</div></div></div>';
    combined += '</div>';
    ...
    for (var counter = 1; counter <= before.tableCount; counter++) {
        ...
        if ($.fn.dataTable && $.fn.dataTable.isDataTable(beforeSelector)) {
            try { $(beforeSelector).DataTable().destroy(); } catch (e) { }
            $(beforeSelector).find('.dataTables_wrapper').remove();
        }
        var beforeOpts = {
            responsive: false,
            autoWidth: false,
            ...
            info: false,
            columnDefs: [
                { targets: [0, 2, 3, 4, 5, 6, 7, 8, 9], className: 'td-column-right' }
            ]
        };
        try {
            $(beforeSelector).DataTable(beforeOpts);
        } catch (e) {
    ...
    // Apply datatables to After results (destroy previous instances to avoid duplicates)
    for (var counter = 1; counter <= after.tableCount; counter++) {
        var afterSelector = '#after_resultTable' + counter;
        if ($.fn.dataTable && $.fn.dataTable.isDataTable(afterSelector)) {
            try { $(afterSelector).DataTable().destroy(); } catch (e) { }
            $(afterSelector).find('.dataTables_wrapper').remove();
        }
        var afterOpts = {
            responsive: false,
            autoWidth: false,
        ...
        try {
            $(afterSelector).DataTable(afterOpts);
        } catch (e) {
            $(afterSelector).dataTable(afterOpts);
        }
    }

    // Synchronize vertical scrolling between the two comparison-scroll containers
    try {
        var scrollContainers = document.querySelectorAll('.comparison-panel .comparison-scroll');
        if (scrollContainers && scrollContainers.length >= 2) {
            var left = scrollContainers[0];
            var right = scrollContainers[1];
    ...
    // After tables initialized, adjust columns and set up resize handler to keep tables fitting
    try {
        function adjustAllTables() {
            if (!window.jQuery || !$.fn.dataTable) return;
            // adjust before tables
            for (var i = 1; i <= before.tableCount; i++) {
                var sel = '#before_resultTable' + i;
                try { if ($.fn.dataTable.isDataTable(sel)) { $(sel).DataTable().columns.adjust(); } } catch (e) { }
            }
            // adjust after tables
            for (var j = 1; j <= after.tableCount; j++) {
                var sel2 = '#after_resultTable' + j;
                try { if ($.fn.dataTable.isDataTable(sel2)) { $(sel2).DataTable().columns.adjust(); } } catch (e) { }
            }
        }
        ...
        window.addEventListener('resize', function () {
            ...
            resizeTimeout = setTimeout(adjustAllTables, 150);
        });

        // initial adjust shortly after render
...
function outputIOTable(statInfo, statTotal, tableNumber, langObj, idPrefix) {
    var prefix = idPrefix || '';
    var result = '<table id="' + prefix + 'resultTable' + tableNumber + '" class="table table-striped table-hover table-condensed" style="table-layout:fixed">';
    result += '<thead><tr>';
...
function clearResult() {
    if (document.getElementById("result").innerHTML != '') {
        ...
        document.getElementById("statiotext_before").value = '';
        document.getElementById("statiotext_after").value = '';
